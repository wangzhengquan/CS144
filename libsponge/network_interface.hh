...
class NetworkInterface {
  private:
    ...
    std::map<uint32_t, EthernetAddressEntry> _ethernet_address_map{};
    std::map<uint32_t, std::shared_ptr<std::deque<InternetDatagram> > > _pending_map{};
    size_t _ms_clock = 0;


    void queue_datagram(const uint32_t next_hop_ip, const InternetDatagram &dgram);
    void real_send_datagram(const InternetDatagram &dgram, const EthernetAddress & next_hop_eth_ddress );
    void send_pending_datagram(uint32_t next_hop_ip, const EthernetAddress & next_hop_eth_ddress);
